  ...
  methods: {
    ...
    async queryList(page) {
      ...
      const res = await $axios.get(
        `/v1/order/material/reports?page=${page}&size=${this.size}`,
        this.queryForm
      );
      if (res.msg === "ok") {
    ...
    async getDetailList(page) {
      page = page || 1;
      const res = await $axios.get(
        `/v1/order/material/report?page=${page}&size=${this.detailSize}&id=${this.detailForm.id}`
      );
      if (res.msg === "ok") {
    ...
    deleteDetail(row) {
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await $axios.post("/v1/order/material/report/delete", {
            id
          });
          if (res.msg === "ok") {
