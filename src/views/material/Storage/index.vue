...
export default {
  ...
  methods: {
    ...
    async exportFile(row) {
      let id = row.id;
      this.$exportExcel(
        "/api/v1/material/exportMaterialReports",
        { report_id: id }
      );
    },
    async queryList(page) {
      page = page || 1;
      const res = await $axios.get(
        `/api/v1/order/material/reports?page=${page}&size=${
          this.size
        }`,
        this.queryForm
      );
    ...
    async getDetailList(page) {
      page = page || 1;
      const res = await $axios.get(
        `/api/v1/order/material/report?page=${page}&size=${
          this.detailSize
      ...
      this.$confirm("此操作将永久删除, 是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await $axios.post(
            "/api/v1/order/material/report/delete",
            {
              id
            }
          );
