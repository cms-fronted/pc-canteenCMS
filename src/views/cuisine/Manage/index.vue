  ...
  methods: {
    async fetchList(page) {
      ...
      const res = await $axios.get(
        `https://tonglingok.com/canteen/api/v1/foods`,
        {
          f_type: 2,
          page: page,
          size: this.pageSize,
    ...
    async getLocationList(company_id) {
      ...
      if (!isNaN(company_id)) {
        if (company_id) {
          ...
          /*   $axios
            .get(`https://tonglingok.com/canteen/api/v1/canteens?company_id=0`)
            .then(res => {
    ...
    async _edit(item) {
      ...
      const res = await $axios.get(
        "https://tonglingok.com/canteen/api/v1/food",
        {
          id: item.id
        }
      );
    ...
    async confirmEdit() {
      ...
      const res = await $axios.post(
        "https://tonglingok.com/canteen/api/v1/food/update",
        data
      );
      if (res.msg === "ok") {
        this.handleClose();
    ...
    handleDelete(item) {
      this.$confirm("此操作将永久删除该商品, 是否继续?", "提示", {
        ...
        .then(async () => {
          await $axios
            .post("https://tonglingok.com/canteen/api/v1/food/handel", {
              id: item.id
            })
            .then(async () => {
