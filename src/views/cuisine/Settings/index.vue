...
export default {
  ...
  methods: {
    ...
    async getLocationList(company_id) {
      ...
      if (!isNaN(company_id)) {
        if (company_id) {
          res = await $axios.get(`/api/v1/canteens?company_id=${company_id}`);
        } else {
    ...
    async addNewMenu() {
      const res = await $axios.post("/api/v1/menu/save", this.menuForm);
      if (res.msg === "ok") {
    ...
    async fetchTableList(page) {
      ...
      const res = await $axios.get(
        `/api/v1/menus/company?company_id=${this.company_id}&canteen_id=${this.canteen_id}&size=${this.size}&page=${page}`
      );
      if (res.msg == "ok") {
    ...
    _delete(row) {
      ...
      this.$confirm("此操作将永久删除该配置, 是否继续?", "提示", {
        ...
        .then(async () => {
          const res = await $axios.post("/api/v1/menu/save", formdata);
          if (res.msg === "ok") {
            this.$message({
              type: "success",
              message: "删除成功!"
